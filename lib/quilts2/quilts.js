	...
	const inputVue = new Vue({
		...
		methods: {
			...
			resizeTexture: function() {
				...
				const empty = new Array(w).fill(0, 0, w).map(() => createSqu([0]));
				if(t[0].length < w) {
					t.forEach((row) => Array.prototype.push.apply(row, empty.slice(row.length)));
				}
				if(t.length < h) {
					while(t.length < h) t.push(empty.slice(0));
				}
				this.update();
			},
