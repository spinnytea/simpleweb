(function() {
	...
	const inputVue = new Vue({
		...
		methods: {
			stepUp: function(id) { Vue.set(this, id, stepUp(id)); this.update(); },
			stepDown: function(id) { Vue.set(this, id, stepDown(id)); this.update(); },
			update: function() {
				this.rebuildTexture();
	...
	const quiltVue = new Vue({
		...
		methods: {
			stepUp: function(id) { Vue.set(this, id, stepUp(id)); },
			stepDown: function(id) { Vue.set(this, id, stepDown(id)); },
	...
	function decodeInputVue(encoded) {
		...
		switch(version) {
			case 1: // d=WzEsMywyMS4yLDM4LjE2LC0yLjEyMTMyMDM0MzU1OTY0LDAsLTQ1LCIjZmYyYjIwLCMyZmVkZGEsI2ZmNGFmNiwjMmU4YjU3LCNiMjIyMjIiLFtbImYtMC0xIiwiZi0xLTIiLCJzLTAiXSxbImYtMS0zIiwicy0yIiwiZi0yLTEiXSxbInMtMyIsImYtMy0xIiwiZi0xLTAiXV1d
			case 2: // d=WzIsMywyMS4yLDM4LjE2LC0yLjEyMTMyMDM0MzU1OTY0LDAsLTQ1LCIjZmYyYjIwLCMyZmVkZGEsI2ZmNGFmNiwjMmU4YjU3LCNiMjIyMjIiLCJmLTAtMSxmLTEtMixzLTA7Zi0xLTMscy0yLGYtMi0xO3MtMyxmLTMtMSxmLTEtMCJd
				// squareSize
				...
				let v1TempTexture = plaintext.pop();
				if(version === 2) v1TempTexture = v1TempTexture.split(';').map((row) => row.split(','));
				data.texture = v1TempTexture.map((row) => row.map((cell) => {
	...
	function share() {
		// TODO put the version outside the url too, in case the encoding/decoding changes
		//  - we can also support the absense of a version as the first type of decoding
		const url = location.protocol + '//' + location.host + location.pathname + '?d=' + encodeInputVue(inputVue._data);
		...
		if (navigator.share) {
			...
			// fallback
			alert('No native share. Copy the url from the new tab, and paste it whereever you\'d like.');
