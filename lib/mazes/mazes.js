		...
		methods: {
			...
			wander: function() {
				...
				let path = findPath((head, visited) => {
					...
					return head.class === 'unvisited' &&
						dirs.every((dir) => !head[dir] || visited.has(head[dir]));
				}, 4);

				if(!path) {
					// find the closest unvisited node
					path = findPath((head) => head.class === 'unvisited');
				}
