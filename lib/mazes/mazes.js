(function() {
	...
	const maze = new Vue({
		...
		methods: {
			...
			makeNewBoard: function() {
				if(this.board && !this.live) return; // TODO remove this restriction (gotta stop the previous generation)
				this.live = false;
				try {
					this.makeEmptyBoard();
				} catch(e) {
	...
	/* explore */
	function findPath(goalFn) {
		const visited = new Set();
		const next = [[maze.active, []]];

		while(next.length) {
			let [head, path] = next.pop();
			...
			dirs.forEach((dir) => {
				...
				if(nextHead && !visited.has(nextHead)) {
					...
					nextPath.push(dir);
					next.push([nextHead, nextPath]);
				}
			});
		}

		// XXX what else is there to say?
		//  - the goal function is opaque
