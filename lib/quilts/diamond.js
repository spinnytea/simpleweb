(function() {
	...
	const inputVue = new Vue({
		...
		data: {
			...
			// TODO change this to be the single square, not the matrix size
			//  - also add toggle for weather it should be a 'single' tl corner, or a 2 square tl corner (right now it's the 2)
			//  - it'd also be nice to have arbitrary offsets to the pattern (say, drag the picture left and right)
			//  - OR -
			//  - do we just need the toggle? because all the paterns i think I want are just that
			//  - (add a tall left, subtract a small right)
			//  - example: clear the square, fill the first column and row
			...
			list: function() {
				...
				for(let cs = 0; cs <= inputVue.cols; cs++) {
					for(let rs = 0; rs < inputVue.rows; rs++) {
						inputVue.matrix.forEach((row, r) => { row.forEach((cell, c) => {
							// truncate left
							if(cs === 0 && r + c + 1 < inputVue.matrix.length) return;
							// truncate right
							if(cs === +inputVue.cols && r + c + 1 > inputVue.matrix.length) return;
