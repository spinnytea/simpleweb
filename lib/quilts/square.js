(function() {
	...
	const quiltVue = new Vue({
		el: 'svg',
		data: {
			list: quilt, // TODO quilt needs to be computed
		},
		computed: {
			quiltWidth: function() { return baseWidth * COLS; },
			quiltHeight: function() { return baseHeight * ROWS; },

			svgWidth: function() { return (this.quiltWidth + 2*MARGIN) * inputVue.zoom; },
			svgHeight: function() { return (this.quiltHeight + 2*MARGIN) * inputVue.zoom; },
			viewBox: function() { return [-MARGIN, -MARGIN, this.quiltWidth+2*MARGIN, this.quiltHeight+2*MARGIN].join(' '); },
		},
	});
